...
exports.execute = function(config) {
	...
	for (i = 0; i < headsLength; i++) {
		row = '<x:row r="' + (i + 1) + '">';
		row += addStringCol(getColumnLetter(1) + (i + 1), heads[i], styleIndex);
		row += '</x:row>';
		rows += row;
	}

	//second row for column caption
	row = '<x:row r="' + (headsLength + 1) + '" spans="1:' + colsLength + '">';
	var colStyleIndex;
	for (k = 0; k < colsLength; k++) {
		colStyleIndex = cols[k].captionStyleIndex || 0;
		row += addStringCol(getColumnLetter(k + 1) + (headsLength + 1), cols[k].caption, colStyleIndex);
		if (cols[k].width) {
	...
	for (i = 0; i < dataLength; i++) {
		r = data[i], currRow = i + 2 + headsLength;
...
var getColumnLetter = function(col) {
	...
	var result = String.fromCharCode.apply(null, array.reverse());
	return result;
};